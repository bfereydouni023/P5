...
class Individual_Grid(object):
    ...
    def mutate(self, genome):
        ...
        # If fitness is high, reduce or skip mutation
        if current_fitness > 8.0 and random.random() > 0.2:
            return genome  # 80% chance to skip mutation
        ...
        for y in range(height):
            for x in range(left, right):
                if (y == 14 and x == 0) or (7 <= y < 14 and x == width - 1):
                    continue
                if genome[y][x] in ("m", "f", "v"):
                    continue

                if random.random() < mutation_rate:
                    new_tile = random.choice(weighted_tiles)

                    if new_tile in ("T", "|"):
                        if y == height - 1 or genome[y + 1][x] != "X":
                            continue
                    genome[y][x] = new_tile
